...
class InsertStepUI:
  def __init__(self, app):
    ...
    # TODO: this should be generated by decorator registration on import
    ...
    if not isinstance(self.app.sequence[self.app.sequence_pos], WhenStep):
      self.inserters.extend(action_inserters)

    step_menu_options = [ui.name for ui in self.inserters]  # type: ignore
    assert len(step_menu_options) == len(set(step_menu_options)), "All step UIs must have unique user-facing names"
  ...
  def _handle_menu_select(self, item, idx):
    ...
    self.ui_delegate = None
    for ui in self.inserters:
      if item == ui.name:  # type: ignore
        self.ui_delegate = ui(self.app)

    assert self.ui_delegate, "UI delegate lookup failed"
